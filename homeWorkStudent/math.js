...
class Hw extends React.Component {
  ...
  FileUpload = (props) => {
    ...
    async function uploadFileToFirebaseStorage(result, file) {
      const name = 'allFiles/subject_Math/'+nameHw+ "/"+file.name;
      console.log(nameHw);
  ...
  render() {
    ...
    nameHw = {text}.text;
    // put data
    this.usersCollectionRef = firestore().collection("subject_Math").doc(nameHw).collection("ans");
    //show data
    this.fireStoreData = firestore().collection("subject_Math").doc(nameHw).collection("homeWorkDetail");
    /////////////////////////////////
